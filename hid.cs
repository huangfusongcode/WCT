...
namespace HID
{
    ...
    public class Hid : object
    {
        ...
        public HID_RETURN OpenDevice(UInt16 vID, UInt16 pID, string serial)
        {
            ...
            {
                //获取连接的HID列表
                List<string> deviceList = new List<string>();   // 定义一个字符串泛型列表

                GetHidDeviceList(ref deviceList);               // 获取每个设备的路径，存放到字符串数组deviceList中

                if (deviceList.Count == 0)                      // 实际包含元素个数
...
//                    IntPtr device = CreateFile(deviceList[i], DESIREDACCESS.GENERIC_READ | DESIREDACCESS.GENERIC_WRITE, 0, 0, CREATIONDISPOSITION.OPEN_EXISTING, 0x40000000, 0);
                    ...
                    if (device != INVALID_HANDLE_VALUE)
                    {
                        ...
                        IntPtr serialBuff = Marshal.AllocHGlobal(512);          // 使非托管的方式，从内存中分配512个字节

                        // 获取设备属性
                        HidD_GetAttributes(device, out attributes);
                        HidD_GetSerialNumberString(device, serialBuff, 512);

                        string deviceStr = Marshal.PtrToStringAuto(serialBuff); // 把非托管内存serialBuff中的字符串，复制到托管字符串deviceStr中
                        Marshal.FreeHGlobal(serialBuff);                        // 释放非托管的内存serialBuff
...
//                        if (attributes.VendorID == vID && attributes.ProductID == pID && deviceStr == serial)
        ...
        //public HID_RETURN Read(report r)
        //{
        ...
        //            hidDevice.Write(buffer, 0, inputReportLength);
        //            // buffert的第1个字节为ID
        //            r.reportID =buffer[0];

        //            int maxBufferLength = 0;
        //            if (r.reportBuff.Length < inputReportLength  - 1)
        //                maxBufferLength = r.reportBuff.Length;
        //            else
        //                maxBufferLength = inputReportLength - 1;

        //            for (int i = 1; i < maxBufferLength; i++)
        //                r.reportBuff[i - 1] =buffer[i];
        ...
        public HID_RETURN Read(report r)
        {
            ...
            if (deviceOpened)
            {
                try
                {
                    ...
                    {
                        MessageBox.Show("aaa");
                        return HID_RETURN.READ_FAILD;

                    }

                    //byte[] buffer = new byte[inputReportLength];
                    hidDevice.Read(buffer, 0, inputReportLength);

                    r.reportID = buffer[0];

                    for (int i = 1; i < inputReportLength; i++)
                        r.reportBuff[i - 1] =buffer[i] ;
